...
default
{
    ...
    touch_start(integer total_number)
    {
        ...
        {
            if(llGetSubString(btn_name, 0, 0) == "_")
            {
                llPlaySound(touch_sound, sound_touch_valume);

                /*
                if(btn_name == "_1" || btn_name == "_2" || btn_name == "_3" || 
                    btn_name == "_4" || btn_name == "_5" || btn_name == "_6")
                {
                    control_options_btn_check(btn_name);
                }
                */
                
                string cmd = llGetSubString(btn_name, 1, llStringLength(btn_name)-1);
                //llOwnerSay(btn_name);
                llWhisper(channel, cmd);
            }
        }
    }
